...
use ascii_dungeon_explorer::systems::{
    CombatFeedbackSystem, ParticleEffectSystem, ScreenShakeState, ScreenShakeSystem,
    SoundEffectSystem, VisualEffectsSystem,
};
        ...
        let _ = with_terminal(|terminal| {
            ...
            if screen_shake.is_shaking() {
                terminal.draw_text_centered(
                    center_y - 6,
                    &format!(
                        "SCREEN SHAKE: {:.1} intensity",
                        screen_shake.current_intensity
                    ),
                    Color::Red,
                    Color::Black,
                )?;
            ...
            Event::Key(key) => {
                match key.code {
                    KeyCode::Char('a') => {
                        ...
                        let current_damage_type =
                            damage_types[damage_type_cycle % damage_types.len()];
                    ...
                    KeyCode::Char('h') => {
                        ...
                        let healing_feedback = CombatFeedback {
                            feedback_type: CombatFeedbackType::HealingText {

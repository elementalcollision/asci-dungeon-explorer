use ascii_dungeon_explorer::map::{
    MapGenerator, RoomBasedDungeonGenerator, DungeonFeatureGenerator, 
    EntityPlacementSystem, TileType, EnemyType, ItemType
};
...
fn main() {
    ...
    with_terminal(|terminal| {
        ...
        // Entity legend
        terminal.draw_text(25, map.height as u16 + 2, "Entity Legend:", Color::White, Color::Black)?;
        terminal.draw_text(25, map.height as u16 + 3, "g - Goblin", Color::Green, Color::Black)?;
        terminal.draw_text(25, map.height as u16 + 4, "o - Orc", Color::Green, Color::Black)?;
        terminal.draw_text(25, map.height as u16 + 5, "s - Skeleton", Color::White, Color::Black)?;
        terminal.draw_text(25, map.height as u16 + 6, "! - Potion", Color::Magenta, Color::Black)?;
        terminal.draw_text(25, map.height as u16 + 7, "$ - Gold", Color::Yellow, Color::Black)?;
        terminal.draw_text(25, map.height as u16 + 8, "C - Chest", Color::Yellow, Color::Black)?;
        
        // Stats
        terminal.draw_text(50, map.height as u16 + 3, 
            &format!("Total Entities: {}", spawns.len()), Color::White, Color::Black)?;
        
        let enemy_count = spawns.iter().filter(|s| matches!(s.entity_type, SpawnType::Enemy(_))).count();
        let item_count = spawns.iter().filter(|s| matches!(s.entity_type, SpawnType::Item(_))).count();
        let special_count = spawns.iter().filter(|s| matches!(s.entity_type, SpawnType::Special(_))).count();
        
        terminal.draw_text(50, map.height as u16 + 4, 
            &format!("Enemies: {}", enemy_count), Color::White, Color::Black)?;
        ...
        terminal.draw_text(50, map.height as u16 + 6, 
            &format!("Special: {}", special_count), Color::White, Color::Black)?;
...
fn entity_glyph_and_color(entity_type: &SpawnType) -> (char, Color) {
    match entity_type {
        ...
        SpawnType::Special(special_type) => {
            match special_type {

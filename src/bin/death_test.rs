...
use ascii_dungeon_explorer::systems::{
    DeathPenaltySystem, GameOverSystem, PlayerDeathSystem, RevivalSystem,
};
            ...
            Event::Key(key) => {
                match key.code {
                    ...
                    KeyCode::Char('r') => {
                        ...
                        if let (Some(death_state), Some(stats)) =
                            (death_states.get_mut(player), combat_stats.get_mut(player))
                        {
                            if death_state.is_dead && death_state.can_revive() {
                                if death_state.revive() {
                                    stats.hp = stats.max_hp / 4; // Revive with 25% HP
                                    game_log.add_entry(
                                        "You have been revived with penalties!".to_string(),
                                    );
                                }
                            }
                        }
                    ...
                    KeyCode::Char('m') => {
                        ...
                        if let (Some(settings), Some(death_state)) =
                            (game_settings.get_mut(player), death_states.get_mut(player))
                        {
                            ...
                            settings.permadeath_enabled =
                                settings.game_mode == GameMode::Permadeath;

                            let mut game_log = world.write_resource::<GameLog>();
                            game_log.add_entry(format!(
                                "Game mode changed to: {}",
                                settings.game_mode.name()
                            ));
                        }
                    }
                    KeyCode::Char('d') => {
                                ...
                                if let Some(action) = show_death_screen(&world, player) {
                                    match action {
                                        DeathAction::ReviveWithPenalty => {
                                            let mut death_states =
                                                world.write_storage::<DeathState>();
                                            let mut combat_stats =
                                                world.write_storage::<CombatStats>();
                                            let mut game_log = world.write_resource::<GameLog>();

                                            if let (Some(death_state), Some(stats)) = (
                                                death_states.get_mut(player),
                                                combat_stats.get_mut(player),
                                            ) {
                                                if death_state.revive() {
                                                    stats.hp = stats.max_hp / 4;
                                                    game_log.add_entry(
                                                        "Revived with penalty!".to_string(),
                                                    );
                                                }
                                            }
                                        }
                                        ...
                                        DeathAction::GiveUp => {
                                            let mut game_log = world.write_resource::<GameLog>();
                                            game_log.add_entry(
                                                "Game Over - Player gave up.".to_string(),
                                            );
                                        }

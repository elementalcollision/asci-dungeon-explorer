...
use ascii_dungeon_explorer::systems::{
    AbilityCooldownSystem, AbilityTargetingSystem, SpecialAbilitiesSystem,
};
    ...
    while running {
        ...
        let _ = with_terminal(|terminal| {
            ...
            terminal.draw_text_centered(
                center_y - 14,
                &format!(
                    "BATTLEMAGE: HP {}/{} | Mana {}/{} | Stamina {}/{}",
                    player_stats.hp,
                    player_stats.max_hp,
                    player_resources.mana,
                    player_resources.max_mana,
                    player_resources.stamina,
                    player_resources.max_stamina
                ),
                Color::Green,
                Color::Black,
            )?;
            ...
            terminal.draw_text_centered(
                center_y - 12,
                &format!(
                    "ENEMIES: Orc1 {}/{} HP | Orc2 {}/{} HP | Goblin {}/{} HP",
                    orc1_stats.hp,
                    orc1_stats.max_hp,
                    orc2_stats.hp,
                    orc2_stats.max_hp,
            ...
            terminal.draw_text_centered(
                center_y - 10,
                &format!(
                    "Selected: {} (Mana: {}, Stamina: {}, Cooldown: {})",
            ...
            if !player_effects.effects.is_empty() {
                let effects_text = player_effects
                    .effects
                    .iter()
        ...
        match read().unwrap() {
            Event::Key(key) => {
                match key.code {
                    KeyCode::Left => {
                        selected_ability = (selected_ability + available_abilities.len() - 1)
                            % available_abilities.len();
                    }
                    KeyCode::Right => {
                    ...
                    KeyCode::Char(' ') => {
                        ...
                        let current_ability =
                            available_abilities[selected_ability % available_abilities.len()];
                        let mana_cost = current_ability.get_mana_cost();
                        let stamina_cost = current_ability.get_stamina_cost();

...
use ascii_dungeon_explorer::systems::{ExperienceSystem, LevelUpSystem, ExperienceGainSystem};
...
fn main() {
    ...
    let player = world.create_entity()
        ...
        .with(Skills::new())
        .with(Abilities::new())
        .with(CharacterClass { class_type: ClassType::Fighter })
        .build();
    ...
    let enemy = world.create_entity()
        ...
        .with(Monster {})
        .with(Name { name: "Test Goblin".to_string() })
        .with(CombatStats {
            max_hp: 15,
    ...
    while running {
        ...
        let _ = with_terminal(|terminal| {
            ...
            // Draw title
            terminal.draw_text_centered(center_y - 15, "CHARACTER PROGRESSION TEST", Color::Yellow, Color::Black)?;
            ...
            terminal.draw_text_centered(
                center_y - 8,
                &format!("Attribute Points: {} | Skill Points: {}", 
                    player_attrs.unspent_points, 
                    world.read_storage::<Skills>().get(player).unwrap().unspent_skill_points),
                Color::White,
                Color::Black
            )?;
                        ...
                        if let Some(action) = show_character_progression(&world, player) {
                            match action {
                                ProgressionAction::AllocateAttribute(attr_type) => {
                                    ...
                                    if let (Some(attr), Some(exp)) = (attributes.get_mut(player), experiences.get_mut(player)) {
                                        if exp.unspent_points > 0 {
                                            if attr.increase_attribute(attr_type) {
                                                exp.unspent_points -= 1;
                                                game_log.add_entry(format!("Increased {:?}!", attr_type));
                                            }
                                        }
                                    }
                                },
                                ProgressionAction::AllocateSkill(skill_type) => {
                                    ...
                                    if let Some(skill) = skills.get_mut(player) {
                                        if skill.increase_skill(skill_type) {
                                            game_log.add_entry(format!("Increased {} skill!", skill_type.name()));
                                        }
                                    }
                                },
                                ProgressionAction::UseAbility(ability_type) => {
                                    ...
                                    if let Some(ability) = abilities.get_mut(player) {
                                        if ability.has_ability(ability_type) && !ability.is_on_cooldown(ability_type) {
                                            ability.set_cooldown(ability_type, ability_type.cooldown());
                                            game_log.add_entry(format!("Used {}!", ability_type.name()));
                                        }
                                    }
                                },

...
use ascii_dungeon_explorer::systems::{
    CombatResolutionSystem, CriticalChanceSystem, CriticalHitSystem, DamageTypeSystem,
    ResistanceManagementSystem,
};
    ...
    while running {
        ...
        let _ = with_terminal(|terminal| {
            ...
            let enemy_stats = world
                .read_storage::<CombatStats>()
                .get(fire_elemental)
                .unwrap();
            let enemy_attacker = world
                .read_storage::<Attacker>()
                .get(fire_elemental)
                .unwrap();
            let enemy_defender = world
                .read_storage::<Defender>()
                .get(fire_elemental)
                .unwrap();
            ...
            // Draw resistances
            let player_resist = world
                .read_storage::<DamageResistances>()
                .get(player)
                .unwrap();
            let enemy_resist = world
                .read_storage::<DamageResistances>()
                .get(fire_elemental)
                .unwrap();
            ...
            terminal.draw_text_centered(
                center_y - 7,
                &format!(
                    "Player Fire Resist: {}% | Enemy Ice Vuln: {}%",
            ...
            let enemy_effects = world
                .read_storage::<StatusEffects>()
                .get(fire_elemental)
                .unwrap();
            ...
            Event::Key(key) => {
                match key.code {
                    ...
                    KeyCode::Char('e') => {
                        ...
                        let mut wants_attack = world.write_storage::<WantsToAttack>();
                        wants_attack
                            .insert(fire_elemental, WantsToAttack { target: player })
                            .expect("Failed to insert attack intent");
                    ...
                    KeyCode::Char('t') => {
                        ...
                        game_log
                            .add_entry(format!("Switched to {} damage type!", current_type.name()));
                    }
                    KeyCode::Char('r') => {
                        // Start new round
                        combat_round += 1;

...
use ascii_dungeon_explorer::systems::{
    AbilityUsageSystem, ResourceRegenerationSystem, StatusEffectSystem,
};
use ascii_dungeon_explorer::ui::{
    draw_resource_bars, show_resource_management_screen, ResourceAction,
};
            ...
            Event::Key(key) => {
                match key.code {
                    KeyCode::Char('r') => {
                        ...
                        if let (Some(resource), Some(stats)) =
                            (resources.get_mut(player), combat_stats.get_mut(player))
                        {
                            ...
                            game_log.add_entry(format!(
                                "You rest and recover {} HP, {} mana, {} stamina.",
                    ...
                    KeyCode::Char('m') => {
                        ...
                        if let Some(resource) = resources.get_mut(player) {
                            ...
                            if resource.consume_stamina(stamina_cost) {
                                ...
                                resource.restore_mana(mana_restore);
                                game_log.add_entry(format!(
                                    "You meditate and restore {} mana for {} stamina.",
                    ...
                    KeyCode::Char('s') => {
                        ...
                        if let Some(effects) = status_effects.get_mut(player) {
                            ...
                            game_log.add_entry(
                                "You feel energized! Stamina regeneration boosted.".to_string(),
                            );
                        }
